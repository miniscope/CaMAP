...
The CLI exposes three commands. Auto-generated flag reference lives in the [CLI Reference](cli.md); this page covers what each command is *for* and how they fit together. Run `camap <command> --help` for the full flag list.
...
## `camap define-zones` — draw the maze graph (maze only)
...
```bash
camap define-zones -d data_paths.yaml --rooms 3 --arms 4
```
...
## `camap detect-zones` — project trajectory onto the graph (maze only)
...
```bash
camap detect-zones -d data_paths.yaml
```
...
`camap analysis` calls this automatically when the cached CSV is missing, so you only need to invoke it directly when:
...
1. `camap define-zones -d data_paths.yaml --rooms <n> --arms <n>` — draw polygons (one-time per camera setup).
2. `camap detect-zones -d data_paths.yaml` — produces `zone_tracking.csv` and the validation video. Skip this if you trust the cached CSV.
3. `camap analysis -c config.yaml -d data_paths.yaml` — full pipeline.
